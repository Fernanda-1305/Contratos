...
- Prazo conta da segunda-feira seguinte à data de aprovação da gerência
- Fim de Cotação = Início de Cotação + 15 dias (referência, editável)
- Elaboração Contrato = campo de status (Concluído/Em andamento)
- Prazo de Assinatura = campo editável (sem fórmula)
...
DIAS_JANELA_COTACAO      = 15   # referência; Fim de Cotação é editável

# ── COLUNAS MANUAIS (preservadas no merge) ────────────────
COLUNAS_MANUAIS = [
...
# ── COLUNAS FINAIS (ordem exata da Base 2) ────────────────
COLUNAS_BASE2 = [
    "Solicitação",
    "Data solicitada",
...
ELABORACAO_OPTS = ["","Concluído","Em andamento"]
...
def dias_entre(d1, d2):
    try:
        if not isinstance(d1, date) or not isinstance(d2, date): return None
        return (d2 - d1).days
    except Exception:
        return None
...
# ── PARSER BASE 2 (Excel exportado do app) ────────────────

def parse_base2_exportado(uploaded_file):
    """
    Lê o Excel exportado pelo próprio app e devolve um DataFrame
    com as colunas de COLUNAS_BASE2. Os campos editáveis já preenchidos
    são preservados — o merge vai priorizar esses valores.
    """
    try:
        ...
        raise ValueError(f"Erro ao ler Excel exportado: {e}")

# ── CÁLCULOS DERIVADOS ────────────────────────────────────

def calcular_derivados(df):
    """
    Recalcula apenas os campos automáticos.
    Nunca sobrescreve colunas manuais que já têm valor.
    """
    ...
    def prazo_resp(row):
        ...
        try:
            return str(max((hoje - proxima_segunda(base)).days, 0))
        except:
            return ""
    ...
    def calc_status(row):
        try:
            ...
            if assinado == "assinado": return "Concluído"
            elab = str(row.get("Elaboração Contrato","")).strip().lower()
            if elab == "em andamento": return "Em andamento"
    ...
    df["Prazo Resposta (dias)"] = df.apply(prazo_resp, axis=1)
    df["Fim de Cotação"]        = df.apply(fim_cot_ref, axis=1)
...
def merge_historico(historico, novos, fonte="erp"):
    """
    fonte='erp'  → novos vêm do relatório ERP (parse_base1)
    fonte='app'  → novos vêm do Excel exportado (parse_base2_exportado)
                   nesse caso os campos manuais preenchidos são aplicados
                   sobre o histórico existente.
    """
    if historico.empty:
        calc = calcular_derivados(novos)
...
def calc_media_total(df):
    ...
    for _, row in df.iterrows():
        try:
            ...
            d2 = safe_parse_date(row.get("Data de Assinatura",""))
...
if menu == "📊  Análise de Contrato":
    ...
    with col_up:
        label_up = ("Arraste o(s) .xlsx do ERP"
                    if "ERP" in tipo_import
                    else "Arraste o Excel exportado pelo app")
    ...
    if uploaded_files:
            ...
            try:
                if "ERP" in tipo_import:
                    ...
                    if novos_df.empty:
                        ...
                        continue
                    df_atual, _ = merge_historico(df_atual, novos_df, fonte="app")
                    atualizacoes += len(novos_df)
            except Exception as e:
                erros.append(f"❌ {uf.name}: {e}")

        st.session_state.historico = df_atual
        historico = df_atual
        df_fil    = (historico if obra_sel == "Todas as obras"
                     else historico[historico["Obra"] == obra_sel])
        salvar(df_atual)
        ...
        if "ERP" in tipo_import:
            if total_novas == 0 and not erros:
                ...
                st.success(f"✅ {total_novas} nova(s) solicitação(ões) importada(s)!")
                st.rerun()
        else:
            if atualizacoes > 0 and not erros:
                st.success(f"✅ Campos editáveis de {atualizacoes} linha(s) atualizados com sucesso!")
                st.rerun()
    ...
    acima30     = sum(1 for v in df_fil["Prazo Resposta (dias)"]
                      if str(v).lstrip("-").isdigit() and int(float(v))>30) if not df_fil.empty else 0
...
# ═════════════════════════════════════════════════════════
    ...
    if df_fil.empty:
        ...
        COLS_VIEW = [
            ...
            "Prazo Resposta (dias)","Início de Cotação","Fim de Cotação",
            "Status Cotação","Data da Definição do fornecedor",
            "Data Elaboração Contrato","Elaboração Contrato",
            "Data de Assinatura","Prazo de Assinatura",
            "Contrato Assinado","Status",
        ]

        with tab_view:
            st.dataframe(
                df_view[[c for c in COLS_VIEW if c in df_view.columns]],
        ...
        with tab_edit:
            st.markdown(
                "<div style='font-size:10px;color:#4a80aa;margin-bottom:9px;'>"
                "Edite os campos manuais e clique em <strong>Salvar</strong>. "
                "Campos em cinza são somente leitura.</div>",
                unsafe_allow_html=True
            )
            ...
            df_ed = st.data_editor(
                df_view[[c for c in COLS_EDIT if c in df_view.columns]].reset_index(drop=True).copy(),
                column_config={
